...
/* v50: enrutador de importación sincronizado con hojas canónicas. */
function _apiImport_(module,rows,userId,batchId){module=Number(module);rows=Array.isArray(rows)?rows:[];var lock=LockService.getScriptLock();try{lock.waitLock(30000);var result;if(module===2||module===3||module===4){var s=_lduV50UpsertStock_(rows,userId);result=ok_({processed:rows.length,imported:s.inserted,inserted:s.inserted,updated:s.updated,rejectedRows:s.rejectedRows,errorCount:0},'Importación sincronizada con LDU_Stock.');}else if(module===1){result=ok_(importIncidents_(rows,userId));}else{result=ok_(importDevices_(rows,userId));try{_lduV50NormalizeDeviceMetadata_(rows,userId);}catch(metadataError){}}clearLduReadCache_();return result;}finally{try{lock.releaseLock();}catch(e){}}}
/* v53: cada importación Stock escribe en su hoja correspondiente. */
function _apiImport_(module,rows,userId,batchId){module=Number(module);rows=Array.isArray(rows)?rows:[];var lock=LockService.getScriptLock();try{lock.waitLock(30000);var result;if(module===2||module===3||module===4){var target=module===3?(LDU.SHEETS.STOCK_A||'LDU_Stock_A'):module===4?(LDU.SHEETS.STOCK_B||'LDU_Stock_B'):(LDU.SHEETS.STOCK||'LDU_Stock'),s=_lduV51UpsertStockTo_(rows,userId,target);result=ok_({processed:rows.length,imported:s.inserted,inserted:s.inserted,updated:s.updated,rejectedRows:s.rejectedRows,errorCount:0,targetSheet:target},'Importación sincronizada con '+target+'.');}else if(module===1){result=ok_(importIncidents_(rows,userId));}else{result=ok_(importDevices_(rows,userId));try{_lduV50NormalizeDeviceMetadata_(rows,userId);}catch(metadataError){}}clearLduReadCache_();return result;}finally{try{lock.releaseLock();}catch(e){}}}
/* v56: todas las secciones de importación usan upsert por IMEI. */
function _apiImport_(module,rows,userId,batchId){module=Number(module);rows=Array.isArray(rows)?rows:[];var lock=LockService.getScriptLock();try{lock.waitLock(30000);var result,target,s;if(module===2||module===3||module===4){target=module===3?(LDU.SHEETS.STOCK_A||'LDU_Stock_A'):module===4?(LDU.SHEETS.STOCK_B||'LDU_Stock_B'):(LDU.SHEETS.STOCK||'LDU_Stock');s=_lduV51UpsertStockTo_(rows,userId,target);result=ok_({processed:rows.length,imported:s.inserted,inserted:s.inserted,updated:s.updated,rejectedRows:s.rejectedRows,errorCount:0,targetSheet:target},'Importación sincronizada con '+target+'.');}else if(module===1){target=LDU.SHEETS.INCIDENTS||'LDU_Incidencias';s=_lduV55UpsertSheet_(rows,userId,target,true);result=ok_({processed:rows.length,imported:s.inserted,inserted:s.inserted,updated:s.updated,rejectedRows:s.rejectedRows,errorCount:0,targetSheet:target},'Importación sincronizada con '+target+'.');}else{target=LDU.SHEETS.DEVICES||'LDU_Dispositivos';s=_lduV55UpsertSheet_(rows,userId,target,false);try{_lduV50NormalizeDeviceMetadata_(rows,userId);}catch(metadataError){}result=ok_({processed:rows.length,imported:s.inserted,inserted:s.inserted,updated:s.updated,rejectedRows:s.rejectedRows,errorCount:0,targetSheet:target},'Importación sincronizada con '+target+'.');}clearLduReadCache_();return result;}finally{try{lock.releaseLock();}catch(e){}}}

/* v58: el handler usa esta asignación final y no una declaración duplicada. */
_apiImport_=lduImportActive_;
