...
(function () {
   ...
   * ------------------------------------------------------------------- */
  /* v108: pedido del usuario — al igual que Inventario LDU/Modelos A/B/Incidencias, tocar una
   * tarjeta de esta sección debe llevar al módulo y filtrar por ese estado (no solo mostrar el
   * número). Reutiliza el mismo mecanismo que window.lduGoToModuleFilter (ldu-v108-correcciones.js). */
  function clickableNoVivoCard(label, value, stateName) {
    return '<article class="metric metric-clickable" data-ldu-goto="imeiNoVivo" data-ldu-selector="#nv-f-estado" data-ldu-value="' + safe(stateName) + '"><strong>' + safe(String(value)) + '</strong><span>' + safe(label) + '</span></article>';
  }
  function imeiNoVivoCards(rows) {
    ...
    return card('📊 TOTAL', rows.length)
      + clickableNoVivoCard('✅ ACTIVOS', byState('Activo'), 'Activo')
      + clickableNoVivoCard('🏬 ALMACÉN', byState('Almacén'), 'Almacén')
      + clickableNoVivoCard('💥 DAÑADOS', byState('Dañado'), 'Dañado')
      + clickableNoVivoCard('🔧 REPARACIÓN', byState('En Reparación'), 'En Reparación')
      + clickableNoVivoCard('🔎 PERDIDOS', byState('Perdido'), 'Perdido')
      + clickableNoVivoCard('⛔ BAJA', byState('Baja'), 'Baja')
      + clickableNoVivoCard('📦 PEND. DEVOLUCIÓN', byState('Pendiente Devolución'), 'Pendiente Devolución')
      + clickableNoVivoCard('↩️ DEVUELTOS', byState('Devuelto'), 'Devuelto')
      ...
      function draw() {
        ...
        document.querySelector('#nv-body').innerHTML = pageRows.map(function (x) {
          return '<tr>' + LDU_COLS.map(function (f) {
            ...
            if (f[0] === 'tipo_uso') return '<td>' + badgeTipoUso(x.tipo_uso) + '</td>';
