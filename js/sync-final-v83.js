...
(function () {
  ...
  function totalsFixed(rows) {
    ...
    var noVivo = stock.filter(function (r) { return upper(r.en_inventario) === 'NO VIVO'; }).length;
    var inInventory = rows.length ? rows.filter(function (r) { return !['DEVUELTO', 'BAJA'].includes(rowState(r)); }).length : stock.filter(function (r) { return upper(r.en_inventario) === 'SI'; }).length;
    return { stock: rows === state.devices ? stock.length : rows.length, enInv: inInventory, act: count('Activo'), alm: count('Almacén'), dan: count('Dañado'), rep: count('En Reparación'), per: count('Perdido'), baja: count('Baja'), pdev: count('Pendiente Devolución'), dev: count('Devuelto'), falt: noInventory, noInv: noInventory, noVivo: noVivo, valor: rows.reduce(function (sum, r) { return sum + number(r.monto !== '' && r.monto != null ? r.monto : r.valor); }, 0) };
  }
  ...
  function renderStockFixed() {
    ...
    api('listStock').then(function (response) {
      ...
      function draw() {
        ...
        document.querySelectorAll('[data-stock-action]').forEach(function (button) { button.onclick = function () { if (button.dataset.stockAction === 'history') { state.historyImei = button.dataset.id; navigate('history'); } else showDetail(rows.find(function (x) { return String(x.imei) === String(button.dataset.id); })); }; });
      }
