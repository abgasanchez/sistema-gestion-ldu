...
(function () {
  ...
  window.previewImport = async function (file, module, card) {
    ...
    drawer.querySelector('[data-confirm]').onclick = async function () { var b = this, p = drawer.querySelector('[data-progress]'), s = drawer.querySelector('[data-status]'), out = drawer.querySelector('[data-result]'); b.disabled = true; s.textContent = 'PROCESANDO ' + rows.length + ' REGISTROS...'; try { var r = await api('importRows', { module: module, rows: rows, userId:lduUserId(), batchId: 'imp89-' + Date.now() }), d = r.data || {}; p.value = 100; s.textContent = '✅ FINALIZADO'; out.innerHTML = '<strong>RESUMEN:</strong> Procesados: ' + rows.length + ' · Nuevos: ' + (d.imported || d.inserted || 0) + ' · Actualizados: ' + (d.updated || 0) + ' · Rechazados: ' + ((d.rejectedRows || []).length); if (r.status === 'ok' && !(d.rejectedRows || []).length) setTimeout(function () { close(); navigate(module === 1 ? 'incidents' : module >= 2 ? 'stock' : 'inventory'); }, 700); } catch (e) { s.textContent = '❌ ERROR'; out.textContent = e.message; } finally { b.disabled = false; } };
  };
}());
