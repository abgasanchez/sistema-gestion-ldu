(function () {
  ...
  api = async function (action, payload) {
    if (action === 'importRows') {
      payload = payload || {};
      var legacyAction = payload.module === 1 ? 'importarIncidencias' : payload.module === 3 ? 'importarStock' : payload.module === 4 ? 'importarStock' : 'importarInventarioAuto';
      var legacyPayload = { rows: payload.rows || [], usuario: payload.userId || 'importacion', sync: '1' };
      if (payload.module === 2 || payload.module === 3 || payload.module === 4) legacyPayload.gid = payload.module === 3 ? 'grpa' : payload.module === 4 ? 'grpb' : '';
      var legacyResult = await apiBase33(legacyAction, legacyPayload);
      if (legacyResult && legacyResult.data) {
        legacyResult.data.imported = Number(legacyResult.data.inserted || 0);
        legacyResult.data.rejected = Number(legacyResult.data.errores || (legacyResult.data.errors || []).length || 0);
      }
      return legacyResult;
    }
  ...
  function ensureChrome33() {
    ...
    var shell = document.querySelector('.app-shell'), main = document.querySelector('.main-content');
    if (shell && main) {
      var footer = document.querySelector('#ldu-footer');
      if (!footer) { footer = document.createElement('footer'); footer.id = 'ldu-footer'; footer.textContent = '© Andy Sanchez. Todos los derechos reservados. Sistema desarrollado para la gestión y control de información.'; }
  ...
  async function groupRows33(gid, label) {
    try {
      var response = await apiBase33('getGrupo', { gid: gid });
      return (response && response.data || []).map(function (x) {
        return { imei: x.imei || x.IMEI, marca: x.marca || x.Marca, modelo: x.modelo || x.Modelo, n_linea: x.n_linea || x.NLinea, responsable: x.responsable || x.Responsable || x.Nombre, dni: x.dni || x.DNI, cargo: x.cargo || x.Cargo, tipo: x.tipo || x.Tipo, supervisor: x.supervisor || x.Supervisor, zona: x.zona || x.Zona, cuenta: x.cuenta || x.Cuenta, departamento: x.departamento || x.Region || x.Departamento, city: x.city || x.City, canal: x.canal || x.Canal, tienda: x.tienda || x.Tienda, tipo_uso: x.tipo_uso || x.TipoUso, fecha_asignacion: x.fecha_asignacion || x.FechaAsignacion, monto: x.monto || x.Monto, estado: x.estado || x.Estado, modulo: label };
      });
    } catch (_) { return []; }
  }
  ...
  window.renderNotifications = async function () {
    ...
    try {
      var all = await loadUnified(), groups = await Promise.all([groupRows33('grpa', 'Modelos Ant. A'), groupRows33('grpb', 'Modelos Ant. B')]), result = await api('listNotifications'), selected = null;
      all = all.concat(groups[0], groups[1]).filter(function (row, index, list) { return list.findIndex(function (other) { return N33(other.modulo) === N33(row.modulo) && I33(other.imei) === I33(row.imei); }) === index; });
