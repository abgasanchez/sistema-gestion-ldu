...
(function () {
  ...
  var ROLE_ACTIONS_ = {
    ...
    uploadIncidentAttachment: ['Administrador', 'Supervisor', 'Operador'],
   ...
   * v128: mismo arreglo que _lduCheckPermission_ (api.gs) — comparación sin distinguir
   * mayúsculas/minúsculas, para que un rol guardado como "ADMINISTRADOR" (o cualquier otra
   * variante de mayúscula) siga reconociéndose como 'Administrador'. */
  function lduCan_(action) { var allowed = ROLE_ACTIONS_[action]; if (!allowed) return true; var role = lduRole_().toLowerCase(); return allowed.some(function(r) { return r.toLowerCase() === role; }); }
  ...
  /* v108: _lduCheckPermission_ (api.gs) responde "No se pudo verificar tu usuario para esta
   * acción. Vuelve a iniciar sesión." cuando el userId que manda el frontend (lduUserId(),
   * leído de sessionStorage['ldu-session']) ya no coincide con ningún usuario real en
   * LDU_Usuarios — típicamente porque sessionStorage se perdió (pestaña nueva, sessionStorage
   * no sobrevive el cierre de pestaña) sin que la interfaz lo note; el usuario se queda
   * viendo ese mensaje suelto dentro del formulario que estaba llenando, sin ninguna forma de
   * recuperarse salvo adivinar que debe recargar la página. Ahora, cualquier llamada que
   * reciba EXACTAMENTE ese mensaje limpia la sesión local y fuerza la pantalla de login —
   * la propia sugerencia del mensaje, aplicada en vez de solo mostrada. */
  var baseApiV108 = window.api;
  if (typeof baseApiV108 === 'function') {
    window.api = api = async function (action, payload) {
      var result = await baseApiV108(action, payload);
      if (result && result.status !== 'ok' && /Vuelve a iniciar sesión/i.test(result.message || '')) {
