...
(function () {
  ...
  if (typeof baseRenderDashboard108_ === 'function') {
    window.renderDashboard = renderDashboard = async function () {
      try { var r = await api('listImeiNoVivo'); state.imeiNoVivo = (r && r.data) || []; }
      catch (e) { state.imeiNoVivo = state.imeiNoVivo || []; }
      return baseRenderDashboard108_.apply(this, arguments);
    };
  }
  ...
  if (typeof baseTotals108_ === 'function') {
    window.totals = totals = function (rows, group) {
      if (rows === undefined && group === undefined) {
        var merged = (state.devices || []).concat(state.imeiNoVivo || []);
        var t = baseTotals108_(merged);
        t.__general = true;
        t.noVivoCount = (state.imeiNoVivo || []).length;
        return t;
      }
  ...
  window.renderSearch = async function () {
    ...
    try {
      ...
      var draw = function () {
        ...
        document.querySelector('#v25-search-cards').innerHTML = q && out[0] ? '<div class="cards module-cards">' + card('📦 MÓDULO', out[0].modulo) + card('📱 IMEI', out[0].imei) + card('📊 ESTADO', out[0].estado || out[0].estado_proceso || '—') + card('🏢 CUENTA', out[0].cuenta || '—') + card('📱 MODELO', out[0].modelo || '—') + card('📋 TIPO', out[0].tipo || '—') + '</div>' : '';
