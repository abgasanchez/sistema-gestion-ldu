...
(function () {
   ...
   * (America/Lima, no la del navegador de quien mira la pantalla) y devuelve solo la fecha
   * (AAAA-MM-DD) para campos de fecha simple, o fecha y hora para los de auditoría
   * (creado_en/actualizado_en/fecha_hora/timestamp), nunca el formato ISO con T/Z. */
  ...
  function lduFormatDate119_(value, key) {
    ...
    var p = lduPeruParts119_(d), dateOnly = p.year + '-' + p.month + '-' + p.day;
    var isTimestamp = /creado_en|actualizado_en|fecha_hora|^timestamp$/i.test(String(key || ''));
    return isTimestamp ? dateOnly + ' ' + p.hour + ':' + p.minute : dateOnly;
  }
