...
function apiJsonp(action,payload={}){
  return new Promise((resolve,reject)=>{
    ...
    const jsonpTimeoutMs=action==='importRows'?90000:45000;
    const timer=setTimeout(()=>{cleanup();reject(new Error(`Apps Script no respondió en ${jsonpTimeoutMs/1000} segundos.`))},jsonpTimeoutMs);
...
const LDU_JSONP_ACTIONS = new Set(['health','login','logout','getSnapshot','listDevices','listIncidents','listStock','listImeiNoVivo','listHistory','lookupImei','getGrupo','setup','deleteDevice','listDeleted','listNotifications','sendNotification','listUsers','createUser','updateUser','deleteUser','clearSheet','createDevice','createImeiNoVivo','updateDevice','importDeviceRow','importStockRow','importIncidentRow','importDevices','importStock','importIncidents','importarInventarioAuto','importarIncidencias','importarStock']);
...
api = async function(action,payload={}){
  ...
  /* v106: createIncident/updateIncident/uploadIncidentAttachment pueden llevar un adjunto
   * en base64 (hasta ~2MB de texto por archivo). Antes createIncident estaba en
   * LDU_JSONP_ACTIONS, así que ese payload se armaba como querystring de un <script src="...">
   * — muy por encima del límite de longitud de URL de cualquier navegador, así que el script
   * fallaba en cargar (script.onerror) y el usuario veía "No se pudo cargar la respuesta de
   * Apps Script." sin ninguna pista de que el problema era el tamaño del adjunto. POST no
   * tiene ese límite (va en el cuerpo de la solicitud), así que estas acciones van por POST
   * primero, con JSONP solo como respaldo si el POST mismo falla (p. ej. por CORS). */
  if(action==='createIncident'||action==='updateIncident'||action==='uploadIncidentAttachment'){
    try{return await apiPostRequest(action,payload)}catch(postError){
      try{return await apiJsonp(action,payload)}catch(jsonpError){throw new Error('No se pudo guardar: '+jsonpError.message)}
    }
  }
  if(LDU_JSONP_ACTIONS.has(action)) return apiJsonp(action,payload);
  return apiPostRequest(action,payload);
};
/* v106: importRows ahora manda lotes más grandes (final-corrections-v90.js, 60 filas en vez
 * de 5) para reducir la cantidad de llamadas HTTP de una importación masiva — un lote más
 * grande tarda más en escribirse en Apps Script (lee y reescribe la hoja completa por lote,
 * ver repository.gs), así que el timeout fijo de 45s que sirve bien para una lectura normal
 * es más ajustado para ese caso puntual. Solo importRows usa un timeout mayor (90s); el resto
 * de las acciones no cambia. */
async function apiPostRequest(action,payload={}){
  const timeoutMs=action==='importRows'?90000:45000;
  ...
  try{
    ...
    if(error.name==='AbortError')throw new Error(`La API no respondió en ${timeoutMs/1000} segundos.`);
    throw new Error(`No se pudo conectar con Apps Script: ${error.message}`);
  }finally{clearTimeout(timer)}
}
