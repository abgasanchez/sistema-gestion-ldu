const DEMO_DEVICES=[
  ...
  {device_id:'demo-2',imei:'869584072141925',amarca:'VIVO',modelo:'Y29S',monto:629,estado:'Dañado',responsable:'LUZ PATRICIA GUARDERAS',supervisor:'FRANK PAIVA',zona:'NORTE 4',tienda:'Tienda demo'},
...
api = async function(action,payload={}){
  ...
  if(action==='importRows'){
    if(Array.isArray(payload.rows)&&payload.rows.length>25){
      const combined={status:'ok',data:{imported:0,inserted:0,updated:0,rejectedRows:[],errorCount:0}};
      for(let start=0;start<payload.rows.length;start+=25){
        const part=Object.assign({},payload,{rows:payload.rows.slice(start,start+25),batchId:(payload.batchId||'import')+'-'+start});
        const response=await api('importRows',part),data=response&&response.data||{};
        if(response&&response.status!=='ok')return response;
        combined.data.imported+=Number(data.imported||data.inserted||data.nuevos||0); combined.data.inserted+=Number(data.inserted||0); combined.data.updated+=Number(data.updated||data.actualizados||0); combined.data.errorCount+=Number(data.errorCount||data.errorsCount||0); combined.data.rejectedRows=combined.data.rejectedRows.concat(data.rejectedRows||[]);
      }
      return combined;
    }
