...
/* v49: paginación estandarizada — antes cada módulo con tabla (Stock, Incidencias,
 * Inventario/Modelos A/B, Eliminar...) tenía su propio HTML/clases de paginación (algunos
 * "Mostrando X–Y de Z" con botones ‹›, otros "Página X de Y" solo, con marcado distinto en
 * cada caso), así que se veían y se comportaban distinto de un módulo a otro. Este es el
 * único paginador del sistema; los módulos abajo (active-fixes-v98.js, final-v23.js,
 * eliminar-v30.js) llaman a window.lduRenderPager en vez de armar su propio HTML. */
window.lduRenderPager = function (host, page, totalItems, pageSize, onChange) {
  if (!host) return;
  var pages = Math.max(1, Math.ceil(totalItems / pageSize));
  page = Math.min(Math.max(1, page), pages);
  var start = totalItems ? (page - 1) * pageSize + 1 : 0, end = Math.min(page * pageSize, totalItems);
  host.className = 'ldu-pager';
  host.innerHTML = '<span class="ldu-pager-info">Mostrando ' + start + '–' + end + ' de ' + totalItems + '</span><div class="ldu-pager-controls"><button type="button" class="btn secondary" data-ldu-pager="prev"' + (page <= 1 ? ' disabled' : '') + '>‹ Anterior</button><span class="ldu-pager-page">Página ' + page + ' de ' + pages + '</span><button type="button" class="btn secondary" data-ldu-pager="next"' + (page >= pages ? ' disabled' : '') + '>Siguiente ›</button></div>';
  var prev = host.querySelector('[data-ldu-pager="prev"]'), next = host.querySelector('[data-ldu-pager="next"]');
  if (prev) prev.onclick = function () { if (page > 1) onChange(page - 1); };
  if (next) next.onclick = function () { if (page < pages) onChange(page + 1); };
};
(function () {
  ...
  /* v102: BUG — un mismo IMEI que existe a la vez en Stock y en Incidencias (el caso normal:
   * el equipo sigue en Stock y además tiene una incidencia registrada) se estaba colapsando
   * en UNA sola fila, indexada solo por IMEI. Como los dispositivos se agregan antes que
   * Stock y Stock antes que Incidencias, esa fila única se quedaba con "modulo":"Stock" (el
   * primero en llegar) para siempre — la incidencia SÍ se fusionaba dentro de esa fila (rellenando
   * campos vacíos), pero como fila quedaba invisible: filtrar por "Incidencias" no la
   * encontraba (su modulo decía "Stock"), y sus campos propios (tipo, modalidad,
   * fecha_incidente, imei_nuevo, doc_autorizacion, documentos_adjuntos) no estaban en la
   * lista de columnas de todos modos. Ahora se indexa por IMEI+módulo, así que un IMEI con
   * historial en varios módulos aparece como una fila POR módulo (como Buscador ya lo hace
   * para IMEIs que solo viven en un módulo) — cada una con sus propios campos completos. */
  function canonicalRows(deviceRows, stockRows, incidentRows) {
    const byKey = new Map(), loose = [];
    /* v110: una fila de Stock casi nunca trae su propio "estado" (ese dato vive en el
     * dispositivo asociado, no en Stock — mismo motivo por el que se corrigió antes el
     * desplegable de Estado en el módulo Stock, active-fixes-v98.js). El Buscador armaba la
     * fila "Stock" de un IMEI sin cruzarla con su dispositivo, así que aunque el dispositivo
     * estuviera "Activo", la fila de Stock de ese mismo IMEI se veía "Sin estado" — dos filas
     * del mismo IMEI mostrando información contradictoria. Se arma aquí un mapa IMEI→estado
     * del dispositivo para completar el estado de la fila de Stock cuando ella misma no trae
     * uno. */
    const deviceStateByImei = new Map();
    (deviceRows || []).forEach(function (row) { var id = String(row.imei || '').replace(/\D/g, ''); if (id && row.estado) deviceStateByImei.set(id, row.estado); });
    function add(row, module) {
      const copy = Object.assign({}, row, { modulo: row.modulo || module, imei: row.imei || row.imei_original || '' });
      /* Una fila de Incidencias no tiene n_linea/monto/estado propios (tiene imei_nuevo/
       * valor/estado_proceso) — se rellenan aquí las columnas genéricas que ya usa el resto
       * del sistema (mismo criterio que loadUnified() en app.js) para no tener que duplicar
       * columnas "N° Línea" y "IMEI Nuevo" una al lado de la otra en Buscador/Notificaciones. */
      if (copy.n_linea === undefined || copy.n_linea === '') copy.n_linea = copy.imei_nuevo || '';
      if (copy.fecha_asignacion === undefined || copy.fecha_asignacion === '') copy.fecha_asignacion = copy.fecha_incidente || '';
      if (copy.monto === undefined || copy.monto === '') copy.monto = copy.valor !== undefined ? copy.valor : copy.monto;
      if (copy.estado === undefined || copy.estado === '') copy.estado = copy.estado_proceso || '';
      if ((copy.estado === undefined || copy.estado === '') && module === 'Stock') copy.estado = deviceStateByImei.get(String(copy.imei || '').replace(/\D/g, '')) || '';
      const id = imei(copy);
      if (!id) { loose.push(copy); return; }
      const key = id + '|' + (copy.modulo || module);
      if (!byKey.has(key)) byKey.set(key, copy);
      else mergeInto(byKey.get(key), copy);
    }
    ...
    return Array.from(byKey.values()).concat(loose);
  }
  ...
  window.renderSearch = renderSearch = async function () {
    ...
    try {
      ...
      const fields = ['modulo', 'imei', 'marca', 'modelo', 'n_linea', 'responsable', 'dni', 'cargo', 'tipo', 'modalidad', 'supervisor', 'zona', 'cuenta', 'departamento', 'ciudad', 'tienda', 'tipo_uso', 'fecha_asignacion', 'fecha_ingreso', 'monto', 'estado', 'doc_autorizacion', 'documentos_adjuntos', 'en_inventario', 'estado_inventario', 'guia', 'observaciones'];
      const cardFields = [['📱 IMEI', 'imei'], ['📱 MODELO', 'modelo'], ['📦 MÓDULO', 'modulo'], ['📊 ESTADO', 'estado'], ['📦 INVENTARIO', 'en_inventario'], ['👤 RESPONSABLE', 'responsable'], ['🪪 DNI', 'dni'], ['👤 SUPERVISOR', 'supervisor'], ['📍 ZONA', 'zona'], ['🏢 CUENTA', 'cuenta'], ['🗺️ DEPARTAMENTO', 'departamento'], ['🏙️ CIUDAD', 'ciudad'], ['🏪 TIENDA', 'tienda'], ['📅 FECHA ASIGNACIÓN', 'fecha_asignacion'], ['📋 MODALIDAD', 'modalidad'], ['📄 DOC. AUTORIZACIÓN', 'doc_autorizacion'], ['💰 MONTO S/', 'monto']];
      ...
      const draw = function () {
        ...
        /* v110: BUG — la tarjeta detallada (17 campos: IMEI/RESPONSABLE/DNI/etc.) solo se
         * mostraba cuando la búsqueda ERA un IMEI completo de 10+ dígitos que matcheaba
         * exacto. Buscar por DNI, nombre, N° de línea, etc. — aunque encontrara un único IMEI
         * inequívoco — siempre caía al resumen genérico (Registros encontrados/Monto total/
         * Criterio), con menos información que buscar por IMEI para el MISMO registro. Ahora
         * el criterio es "todos los resultados son el mismo IMEI" (un IMEI real puede
         * aparecer en más de una fila — Inventario + Stock, por ejemplo — y eso sigue siendo
         * "un solo resultado" desde el punto de vista del usuario); con más de un IMEI
         * distinto se sigue mostrando el resumen, que ahí sí tiene más sentido que el detalle
         * de un registro arbitrario. */
        const distinctImeis = new Set(out.map(function (row) { return String(row.imei || '').replace(/\D/g, ''); }).filter(Boolean));
        const singleMatch = out.length > 0 && distinctImeis.size === 1;
  ...
  window.renderUsers = renderUsers = async function () {
    ...
    try {
      /* v107: listUsers está en LDU_ACTION_ROLES_ (api.gs) — a diferencia de listDevices/
       * listStock/listIncidents, SÍ exige rol (Administrador), y _lduCheckPermission_
       * necesita userId para poder resolver ese rol. Esta llamada nunca mandó userId, así
       * que el backend respondía {status:'error', message:'No se pudo verificar tu
       * usuario...'} — un error "normal" (no una excepción), así que el catch de abajo
       * nunca lo atrapaba: rows quedaba en [] y la tabla se veía vacía sin ningún aviso. */
      const result = await api('listUsers', { userId: lduUserId() });
      if (result.status !== 'ok') throw new Error(result.message || 'No se pudo cargar Usuarios.');
      ...
      document.querySelectorAll('[data-v36-user]').forEach(button => button.onclick = async function () { const user = rows[Number(button.dataset.index)]; if (!user) return; if (button.dataset.v36User === 'edit') return openUserEditor(user); if (button.dataset.v36User === 'toggle') { button.disabled = true; await api('updateUser', { userId: lduUserId(), user: { id_usuario: user.id_usuario, activo: (user.activo === true || ['sí','si','true'].includes(String(user.activo).toLowerCase())) ? 'No' : 'Sí' } }); return renderUsers(); } if (button.dataset.v36User === 'delete' && confirm('¿Eliminar este usuario? Esta acción quedará registrada.')) { button.disabled = true; await api('deleteUser', { userId: lduUserId(), userIdTarget: user.id_usuario }); return renderUsers(); } });
  ...
  function openUserEditor(user) {
    ...
    /* v109: BUG — mandaba userId: user.id_usuario (el UUID del usuario QUE SE ESTÁ EDITANDO)
     * en vez del usuario que hace la petición. api.gs (_lduCheckPermission_) exige que
     * "userId" sea el USUARIO LOGUEADO que ejecuta la acción (busca ese valor contra la
     * columna "usuario" de LDU_Usuarios para resolver su rol) — un UUID nunca calza con
     * ningún "usuario", así que el rol quedaba vacío y SIEMPRE fallaba con "No se pudo
     * verificar tu usuario para esta acción", en el 100% de las ediciones, sin importar la
     * sesión. Confirmado contra el backend en vivo: userId=admin (el que corresponde) sí
     * funciona; userId=<uuid del usuario editado> siempre da ese error. Ahora manda
     * lduUserId() — el mismo criterio que ya usan toggle/eliminar unas líneas arriba. */
    drawer.querySelector('form').onsubmit = async event => { event.preventDefault(); const value = Object.fromEntries(new FormData(event.target).entries()); value.id_usuario = user.id_usuario; if (!value.password) delete value.password; const result = await api('updateUser', { userId: lduUserId(), user: value }); if (result.status !== 'ok') drawer.querySelector('#v36-user-message').textContent = result.message || 'No se pudo actualizar el usuario.'; else { close(); renderUsers(); } };
  }
  ...
  /* v50: rediseño visual de Configuración para que quede igual al resto del sistema — antes
   * envolvía Seguridad/Mantenimiento/Limpieza en <article class="section"> DENTRO de un
   * <section class="section"> exterior (una tarjeta dentro de otra tarjeta, con su propio
   * borde/sombra duplicado), algo que ningún otro módulo hace: todos usan secciones
   * hermanas al nivel superior de #app. También se agrega una fila de tarjetas de estado
   * (rol, acceso, hojas protegidas) igual que el resto de módulos. */
  window.renderSettings = renderSettings = function () {
    ...
    const statusCards = '<div class="cards compact">' + card('👤 USUARIO', (currentUser.username || '—').toUpperCase()) + card('💼 ROL', (currentUser.role || 'Consulta').toUpperCase()) + card('🔐 ACCESO A MANTENIMIENTO', isAdmin ? 'HABILITADO' : 'RESTRINGIDO') + card('🧹 HOJAS PROTEGIDAS', cleanup.length) + '</div>';
    const maintenanceSection = !isAdmin ? '' : '<section class="section"><div class="section-head"><h2>🛠️ Mantenimiento</h2></div><div class="section-body"><div class="toolbar"><button class="btn" id="v36-health">🔌 Probar conexión</button><button class="btn secondary" id="v36-validate">📋 Validar estructura</button><button class="btn secondary" id="v36-reconcile">🔄 Sincronizar Stock por grupos</button><button class="btn secondary" id="v36-standardize">🧹 Estandarizar formatos</button><button class="btn secondary" id="v36-sync-novivo">📵 Sincronizar IMEI NO VIVO</button><button class="btn secondary" id="v36-backfill">🧩 Completar Marca/Modelo/Cuenta</button><button class="btn" id="v36-sync-all">🔄🗂️ Sincronizar Todo el Sistema</button></div></div></section>';
    const cleanupSection = !isAdmin ? '' : '<section class="section"><div class="section-head"><h2>🧹 Limpieza protegida por hoja</h2></div><div class="section-body"><p class="muted">Se conservarán los encabezados. Esta acción elimina los registros de la hoja seleccionada y no se puede deshacer — requiere seleccionar una hoja, ver cuántos registros se afectan y escribir la confirmación exacta.</p><div class="settings-buttons">' + cleanup.map(item => '<button type="button" class="btn danger" data-v36-clean="' + item[0] + '">' + item[1] + '</button>').join('') + '</div></div></section>';
    document.querySelector('#app').innerHTML = '<section class="section"><div class="section-head"><h2>⚙️ Configuración</h2></div><div class="section-body">' + statusCards + (isAdmin ? '' : '<p class="muted">El mantenimiento avanzado y la limpieza de hojas están reservados al rol Administrador.</p>') + '</div></section><section class="section"><div class="section-head"><h2>🔐 Seguridad</h2></div><div class="section-body"><form id="password-form" class="device-form">' + field('Contraseña actual', 'current_password', 'password', '', 'required') + field('Nueva contraseña', 'new_password', 'password', '', 'required minlength="4"') + field('Confirmar contraseña', 'confirm_password', 'password', '', 'required minlength="4"') + '<button class="btn">🔐 Cambiar contraseña</button></form></div></section>' + maintenanceSection + cleanupSection;
    ...
    if (!isAdmin) return;
    /* v119: BUG — estos 5 botones son las únicas acciones de Mantenimiento (todas requieren
     * rol Administrador en LDU_ACTION_ROLES_, api.gs) y NINGUNO mandaba userId en el payload.
     * _lduCheckPermission_ recibe entonces p.userId === undefined, _lduUserRole_(undefined) no
     * puede resolver ningún rol y responde "No se pudo verificar tu usuario para esta acción.
     * Vuelve a iniciar sesión." — active-fixes-v105-roles.js interpreta ESE mensaje exacto
     * como sesión inválida y fuerza el logout, aunque la sesión sí era válida (por eso el
     * usuario veía su ADMIN/ADMINISTRADOR correctos en las tarjetas de arriba un instante
     * antes de que la alerta lo mandara de vuelta al login). Se agrega userId a los 5. */
    var v36MaintUserId_ = typeof lduUserId === 'function' ? lduUserId() : (currentUser.username || '');
    document.querySelector('#v36-health').onclick = function () { api('health', { userId: v36MaintUserId_ }).then(function (r) { alert(r.message || 'Conexión activa'); }).catch(function (e) { alert(e.message); }); };
    document.querySelector('#v36-validate').onclick = function () { api('setup', { userId: v36MaintUserId_ }).then(function (r) { alert(r.message || 'Estructura validada'); }).catch(function (e) { alert(e.message); }); };
    document.querySelector('#v36-reconcile').onclick = function () { api('reconcileStock', { userId: v36MaintUserId_ }).then(function (r) { alert(r.message || 'Stock sincronizado por grupos.'); }).catch(function (e) { alert(e.message); }); };
    /* v119: pedido del usuario — "toda la información...debe estar en MAYÚSCULAS por
     * defecto...tanto la información existente como los nuevos registros", incluido el
     * spreadsheet. Se suma uppercaseAll a este mismo botón (no se crea uno nuevo) porque ya
     * es, conceptualmente, el botón de "normalizar formato de los datos existentes". */
    /* v129: pedido del usuario — "El servicio Hojas de cálculo falló al acceder al documento"
     * es un error transitorio de Google por abrir el mismo spreadsheet grande 3 veces seguidas
     * sin pausa. El backend ya reintenta (_lduOpenSpreadsheet_, utils.gs); aquí además se deja
     * una pausa breve entre cada llamada y se marca con ✅/❌ cuál de las 3 sí terminó bien, en
     * vez de mezclar los 3 mensajes sin distinguir cuál falló. */
    var v36Pause_ = function (ms) { return new Promise(function (r) { setTimeout(r, ms); }); };
    document.querySelector('#v36-standardize').onclick = async function () {
      var button = this; button.disabled = true;
      var lines = [];
      try {
        var dates = await api('formatDates', { userId: v36MaintUserId_ });
        lines.push((dates.status === 'ok' ? '✅ ' : '❌ ') + (dates.message || 'Fechas.'));
        await v36Pause_(2500);
        var money = await api('formatMoney', { userId: v36MaintUserId_ });
        lines.push((money.status === 'ok' ? '✅ ' : '❌ ') + (money.message || 'Montos.'));
        await v36Pause_(2500);
        var upper = await api('uppercaseAll', { userId: v36MaintUserId_ });
        lines.push((upper.status === 'ok' ? '✅ ' : '❌ ') + (upper.message || 'Mayúsculas.'));
        alert(lines.join('\n'));
      } catch (error) { alert((lines.length ? lines.join('\n') + '\n' : '') + '❌ No se pudo continuar: ' + error.message); }
      finally { button.disabled = false; }
    };
    /* v116: pedido del usuario — envía a Inventario/Modelos A/B los IMEI de LDU_IMEI_No_Vivo
     * que ya tienen Stock (corregidos manualmente o con Stock llegado después). */
    document.querySelector('#v36-sync-novivo').onclick = function () { api('syncImeiNoVivo', { userId: v36MaintUserId_ }).then(function (r) { alert(r.message || 'IMEI NO VIVO sincronizado.'); }).catch(function (e) { alert(e.message); }); };
    /* v123: pedido del usuario — completa MARCA (por defecto VIVO), MODELO (desde Stock, por
     * IMEI) y CUENTA (desde el dispositivo asociado, para Stock) donde hoy están en blanco en
     * el spreadsheet — datos que ya existían antes de las correcciones de esta ronda y que
     * esas correcciones, al ser solo hacia adelante, no llegan a arreglar por sí solas. Nunca
     * pisa una celda que ya tenga un valor. */
    document.querySelector('#v36-backfill').onclick = function () { if (!confirm('Esto completará MARCA/MODELO/CUENTA vacíos en Dispositivos, Modelos A/B, Stock y datos de ubicación en Incidencias, usando la información ya cargada en el sistema. No modifica ninguna celda que ya tenga un valor. ¿Continuar?')) return; api('backfillMissingData', { userId: v36MaintUserId_ }).then(function (r) { alert(r.message || 'Datos completados.'); }).catch(function (e) { alert(e.message); }); };
    /* v124: pedido del usuario — un botón que fuerce a todo el sistema a releer las hojas de
     * Google Sheets desde cero, por si algo se editó directamente en el spreadsheet (fuera de
     * la app) y el sistema todavía muestra la copia cacheada. Solo invalida caché — no mueve
     * ni reescribe ningún dato — y recarga la pantalla actual al terminar para que se note el
     * efecto de inmediato. */
    document.querySelector('#v36-sync-all').onclick = function () { var button = this; button.disabled = true; button.textContent = '🔄 Sincronizando...'; api('syncAllFromSheets', { userId: v36MaintUserId_ }).then(function (r) { alert(r.message || 'Sistema sincronizado con las hojas.'); location.reload(); }).catch(function (e) { alert(e.message); button.disabled = false; button.textContent = '🔄🗂️ Sincronizar Todo el Sistema'; }); };
  ...
  window.exportLduAnalysis = function () {
    ...
    exportRows(rows, 'analisis-ldu-' + (selected || 'todos'), LDU_DEVICE_EXPORT_FIELDS);
  };
  ...
  window.renderHistory = renderHistory = async function () {
    ...
    try {
      ...
      const fields = ['fecha','accion','usuario','imei','marca','modelo','n_linea','responsable','dni','cargo','tipo','modalidad','supervisor','zona','cuenta','departamento','ciudad','canal','tienda','tipo_uso','fecha_asignacion','monto','estado','doc_autorizacion','documentos_adjuntos','justificacion','observaciones'];
      /* v132: pedido del usuario — botón de exportar en Historial IMEI, igual patrón que ya
       * usan Buscador/Inventario/Incidencias. Exporta lo que esté cargado en window.__historyRows
       * (el resultado de la búsqueda actual), no toda la hoja completa — es el IMEI consultado. */
      document.querySelector('#app').innerHTML = '<section class="section v36-history"><div class="section-head"><h2>📋 Historial IMEI</h2><div><button class="btn secondary" id="v36-hrefresh">🔄 Actualizar</button><button class="btn secondary" id="v36-hexport">📤 Exportar</button></div></div><div id="v36-hcards"></div><div class="section-body"><input id="v36-hq" class="input search-field" placeholder="🔍 Buscar IMEI completo o parcial..."></div><div id="v36-hresults"></div></section>';
      document.querySelector('#v36-hexport').onclick = function () {
        var out = window.__historyRows || [];
        if (!out.length) { alert('No hay historial para exportar — escribe un IMEI primero.'); return; }
        exportRows(out, 'historial-imei', fields.map(function (f) { return [f, f.replaceAll('_', ' ').toUpperCase()]; }));
      };
      /* v102: esta tarjeta de resumen (la que realmente se redibuja en cada tecleo, vía
       * oninput más abajo) no traía una tarjeta de IMEI — el usuario buscaba un IMEI y no lo
       * veía reflejado en ninguna tarjeta arriba de la tabla — ni los campos propios de un
       * evento de Incidencia (modalidad/doc_autorizacion; estado ya caía a estado_proceso
       * porque cardValue() ya lo hacía, pero no había tarjeta para modalidad/doc_autorizacion). */
      const draw = function () { const query = normalize(document.querySelector('#v36-hq').value); const out = query ? rows.filter(row => normalize(row.imei).includes(query)) : []; const first = out[0] || {}; const total = out.reduce((sum, row) => sum + Number(row.monto || row.valor || 0), 0); document.querySelector('#v36-hcards').innerHTML = out.length ? '<div class="cards compact v36-history-cards">' + [['📱 IMEI','imei'],['📱 MODELO','modelo'],['👤 RESPONSABLE','responsable'],['📊 ESTADO','estado'],['👤 SUPERVISOR','supervisor'],['📍 ZONA','zona'],['🏢 CUENTA','cuenta'],['📋 MODALIDAD','modalidad'],['📄 DOC. AUTORIZACIÓN','doc_autorizacion'],['📅 FECHA','fecha'],['💰 MONTO TOTAL',null]].map(function (item) { return '<div class="metric"><strong>' + (item[1] ? cardValue(first, item[1]) : cardValue({ monto: total }, 'monto')) + '</strong><span>' + item[0] + '</span></div>'; }).join('') + '</div>' : ''; window.__historyRows = out; document.querySelector('#v36-hresults').innerHTML = query ? (out.length ? table(out, fields) : '<div class="empty">No hay historial para ese IMEI.</div>') : '<div class="empty">Escribe un IMEI para consultar el historial.</div>'; };
  ...
  window.renderNotifications = renderNotifications = async function () {
    ...
    const NOTIFICATION_CARD_FIELDS = [['📱 IMEI', 'imei'], ['📱 MODELO', 'modelo'], ['📦 MÓDULO', 'modulo'], ['📊 ESTADO', 'estado'], ['👤 RESPONSABLE', 'responsable'], ['🪪 DNI', 'dni'], ['👤 SUPERVISOR', 'supervisor'], ['📍 ZONA', 'zona'], ['🏢 CUENTA', 'cuenta'], ['🗺️ DEPARTAMENTO', 'departamento'], ['🏙️ CIUDAD', 'ciudad'], ['🏪 TIENDA', 'tienda'], ['📅 FECHA ASIGNACIÓN', 'fecha_asignacion'], ['📋 MODALIDAD', 'modalidad'], ['📄 DOC. AUTORIZACIÓN', 'doc_autorizacion'], ['💰 MONTO S/', 'monto']];
    ...
    const draw = async function () {
      ...
      try {
        ...
        /* v102: antes esta zona SIEMPRE mostraba solo el resumen agregado (conteo/monto/
         * criterio), incluso buscando un IMEI exacto — a diferencia de Buscador, que en ese
         * caso muestra una tarjeta detallada del propio registro. Mismo criterio aquí: si el
         * texto buscado es un IMEI (10+ dígitos) y hay una coincidencia exacta, se muestra el
         * detalle completo de esa fila (por módulo — un IMEI con Stock + Incidencia ahora
         * aparece como dos bloques de tarjetas, uno por cada uno). */
        const digits = q.replace(/\D/g, ''), imeiMatches = digits.length >= 10 ? out.filter(function (row) { return String(row.imei || '').replace(/\D/g, '') === digits; }) : [];
        document.querySelector('#v37-ncards').innerHTML = imeiMatches.length ? imeiMatches.map(function (row) { return '<div class="v37-selected-card"><div class="v37-selected-card-label">📦 ' + safe(row.modulo || '—') + '</div><div class="cards compact v36-card-grid">' + NOTIFICATION_CARD_FIELDS.map(function (field) { return '<div class="metric"><strong>' + cardValue(row, field[1]) + '</strong><span>' + field[0] + '</span></div>'; }).join('') + '</div></div>'; }).join('') : (out.length ? '<div class="cards compact v37-notification-cards"><div class="metric"><strong>' + out.length + '</strong><span>📊 REGISTROS ENCONTRADOS</span></div><div class="metric"><strong>' + formatMoney(out.reduce(function (sum, row) { return sum + Number(row.monto || row.valor || 0); }, 0)) + '</strong><span>💰 MONTO TOTAL</span></div><div class="metric"><strong>' + safe(q) + '</strong><span>🔎 CRITERIO</span></div></div>' : '');
    ...
    document.querySelector('#v37-nform').onsubmit = async function (event) {
      ...
      const result = await api('sendNotification', { notification: { tipo: currentTypeLabel(), asunto: form.get('asunto'), destinatario: recipients.join(','), zona: first.zona || '', rol: first.cargo || '', cc: form.get('cc'), mensaje: html, imeis: selectedEntries.map(function (e) { return e.row.imei; }).filter(Boolean) }, userId:lduUserId()});
  ...
  /* v110: BUG DE RAÍZ del punto "no puedo crear/editar un IMEI que solo está en Stock" —
   * este wrapper de openDeviceForm agregaba un SEGUNDO manejador de blur sobre el mismo campo
   * IMEI (además del que ya trae openDeviceForm en app.js) que llamaba a validateImeiForm().
   * validateImeiForm trataba found = data.device || data.stock como "duplicado" en TODOS los
   * casos, deshabilitaba el guardado (form.dataset.imeiDuplicate='1') y SOBRESCRIBÍA
   * form.onsubmit para bloquear el envío con un alert — sin importar que app.js ya hubiera
   * resuelto correctamente la situación (autocompletar y, si es un dispositivo real, cambiar
   * a modo "Editar"). Resultado: cualquier IMEI que ya existiera en Stock (o incluso como
   * dispositivo real, al abrir "+ Dispositivo" y tipear su IMEI) quedaba con el botón Guardar
   * bloqueado permanentemente, sin ninguna forma de completar su ficha de Inventario. app.js
   * ya cubre este caso de forma correcta por sí solo (ver el comentario de openDeviceForm ahí),
   * así que este wrapper duplicado y conflictivo se elimina en vez de parchearlo. */
  ...
  /* v53: oculta del menú lo que el rol actual no puede usar, según la tabla de permisos real
   * del backend (LDU_ACTION_ROLES_ en api.gs) — esto es solo para no confundir con botones
   * que de todas formas el servidor va a rechazar; quien realmente decide es el backend, no
   * este oscurecimiento visual. No se usa un MutationObserver sobre <body> a propósito (sería
   * caro, se dispararía en cada re-render de cualquier tabla grande) — basta con reaplicar al
   * arrancar y cuando cambia la sesión, igual que ya hace profile() en este mismo archivo. */
  /* v128: mismo arreglo que _lduCheckPermission_ (api.gs) y lduCan_ (active-fixes-v105-roles.js)
   * — antes NAV_HIDDEN_BY_ROLE_[role] y role==='Administrador' eran comparaciones EXACTAS; con
   * 'rol' guardado en mayúsculas (bug ya corregido en repository.gs/UPPERCASE_ALL_SHEETS_V119_,
   * pero ya aplicado a datos existentes) esto hacía que un Administrador real cayera al
   * fallback de Consulta y se le ocultaran Eliminar/Importar/Usuarios. Se normaliza el rol a
   * Título antes de buscarlo en la tabla, sin importar cómo haya quedado guardado. */
  const NAV_HIDDEN_BY_ROLE_ = { Administrador: [], Supervisor: ['delete', 'import', 'users'], Operador: ['delete', 'users'], Consulta: ['delete', 'import', 'users'] };
  const lduNormalizeRole_ = function (role) {
    const key = String(role || '').trim().toLowerCase();
    const found = Object.keys(NAV_HIDDEN_BY_ROLE_).find(function (r) { return r.toLowerCase() === key; });
    return found || 'Consulta';
  };
  const applyRolePermissions_ = function () {
    const role = lduNormalizeRole_(_v36CurrentUser_().role);
    const hidden = NAV_HIDDEN_BY_ROLE_[role] || NAV_HIDDEN_BY_ROLE_.Consulta;
    document.querySelectorAll('.nav-item').forEach(function (btn) { btn.style.display = hidden.indexOf(btn.dataset.view) >= 0 ? 'none' : ''; });
    const addDevice = document.querySelector('.top-action.primary');
    if (addDevice) addDevice.style.display = (role === 'Administrador' || role === 'Operador') ? '' : 'none';
    document.body.dataset.lduRole = role;
  };
  document.addEventListener('DOMContentLoaded', applyRolePermissions_);
  document.addEventListener('ldu:session-changed', applyRolePermissions_);
  setTimeout(applyRolePermissions_, 300); setTimeout(applyRolePermissions_, 1200); setTimeout(applyRolePermissions_, 2500);
})();
