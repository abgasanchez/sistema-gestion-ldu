...
(function () {
  ...
  function _v36CurrentUser_() { let raw = sessionStorage.getItem('ldu-session') || localStorage.getItem('lduUser') || localStorage.getItem('currentUser') || '{}', user = {}; try { user = JSON.parse(raw); } catch (_) {} user = user.data || user; return { username: user.username || user.usuario || 'admin', role: String(user.role || user.rol || '').trim() }; }
  ...
  window.renderNotifications = renderNotifications = async function () {
    ...
    document.querySelector('#v37-nq').oninput = draw; document.querySelector('#v37-nmodule').onchange = draw; document.querySelector('#v37-nrefresh').onclick = renderNotifications; document.querySelector('#v37-nform').onsubmit = async function (event) { event.preventDefault(); const form = new FormData(event.target), recipients = String(form.get('recipient') || '').split(',').map(function (value) { return value.trim(); }).filter(Boolean), msg = document.querySelector('#v37-nmsg'); if (recipients.length < 1 || recipients.length > 4) { msg.textContent = 'Ingresa entre 1 y 4 destinatarios.'; return; } msg.textContent = 'Enviando...'; const result = await api('sendNotification', { notification: { type: form.get('type'), subject: form.get('subject'), recipient: recipients.join(','), cc: form.get('cc'), message: form.get('message'), imeis: selected ? [selected.imei] : [] }, userId: 'web-user' }); msg.textContent = result && result.status === 'ok' ? '✅ Notificación enviada y registrada.' : (result && result.message || 'No se pudo enviar.'); };
  };
  ...
  const installStockTotals = function () { document.querySelectorAll('section').forEach(function (section) { if (!/RESUMEN DE STOCK POR MODELO/i.test(section.textContent || '')) return; section.querySelectorAll('table').forEach(function (tableNode) { const headers = [...tableNode.querySelectorAll('thead th')].map(function (th) { return String(th.textContent || '').trim(); }), body = tableNode.querySelector('tbody'); if (!body) return; [...body.querySelectorAll('tr')].filter(function (row) { return /^TOTAL$/i.test(String(row.cells[0] && row.cells[0].textContent || '').trim()); }).forEach(function (row) { row.remove(); }); const bodyRows = [...body.querySelectorAll('tr')].filter(function (row) { return !row.dataset.v37Total; }); if (!bodyRows.length) return; let total = tableNode.querySelector('tfoot[data-v37-total]'); if (!total) { total = document.createElement('tfoot'); total.dataset.v37Total = '1'; tableNode.appendChild(total); } const sums = headers.map(function () { return 0; }); bodyRows.forEach(function (row) { [...row.cells].forEach(function (cell, index) { if (index === 0) return; const value = Number(String(cell.textContent || '').replace(/[^0-9.-]/g, '')); if (!isNaN(value)) sums[index] += value; }); }); const html = '<tr class="v37-stock-total"><th>TOTAL</th>' + headers.slice(1).map(function (header, index) { const columnIndex = index + 1, money = /MONTO|VALOR/i.test(header), value = sums[columnIndex] || 0; return '<th>' + (money ? formatMoney(value) : String(value)) + '</th>'; }).join('') + '</tr>'; if (total.innerHTML !== html) total.innerHTML = html; }); }); };
