...
(function () {
  ...
  const installStockTotals = function () { document.querySelectorAll('section').forEach(function (section) { if (!/RESUMEN DE STOCK POR MODELO/i.test(section.textContent || '')) return; section.querySelectorAll('table').forEach(function (tableNode) { const bodyRows = [...tableNode.querySelectorAll('tbody tr')].filter(function (row) { return !row.dataset.v37Total; }); if (!bodyRows.length) return; let total = tableNode.querySelector('tfoot[data-v37-total]'); if (!total) { total = document.createElement('tfoot'); total.dataset.v37Total = '1'; tableNode.appendChild(total); } const cells = [...tableNode.querySelectorAll('thead th')].map(function () { return 0; }); bodyRows.forEach(function (row) { [...row.cells].forEach(function (cell, index) { if (index === 0) return; const value = Number(String(cell.textContent || '').replace(/[^0-9.-]/g, '')); if (!isNaN(value)) cells[index] += value; }); }); total.innerHTML = '<tr><th>TOTAL</th>' + cells.slice(1).map(function (value, index) { return '<th>' + (value ? (index >= cells.length - 2 ? 'S/ ' : '') + value.toLocaleString('en-US', { minimumFractionDigits: index >= cells.length - 2 ? 2 : 0, maximumFractionDigits: index >= cells.length - 2 ? 2 : 0 }) : '0') + '</th>'; }).join('') + '</tr>'; }); }); };
  const v37DataObserver = new MutationObserver(function () { autoSelectNotification(); installStockTotals(); }); v37DataObserver.observe(document.querySelector('#app'), { childList: true, subtree: true });
