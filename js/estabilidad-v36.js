...
(function () {
  ...
  function buildNotificationEmailHtml_(typeLabel, messageText, rows) {
    ...
    const first = rows[0] || {}, multi = rows.length > 1;
    const recipientName = rows.length === 0 ? '' : multi ? rows.length + ' registros seleccionados' : (first.responsable || first.nombre || first.imei || '');
    const recipientRole = multi ? '' : (first.cargo || '');
    const recipientZone = multi ? '' : (first.zona || '');
  ...
  window.renderNotifications = renderNotifications = async function () {
    ...
    const renderSelectedSummary = function () { const host = document.querySelector('#v37-nselected-summary'); const list = Array.from(selected.values()); if (!list.length) { host.className = 'notification-selected empty'; host.textContent = 'Ningún registro seleccionado.'; return; } host.className = 'notification-selected'; host.innerHTML = '<b>' + list.length + ' registro' + (list.length === 1 ? '' : 's') + ' seleccionado' + (list.length === 1 ? '' : 's') + ':</b> ' + list.map(function (r) { return safe(r.imei || '—'); }).join(', '); };
    const draw = async function () {
      ...
      try {
        ...
        document.querySelectorAll('[data-v37-select]').forEach(function (box) { box.onchange = function () { const row = out[Number(box.dataset.v37Select)], key = imei(row) || ('row' + box.dataset.v37Select); if (box.checked) selected.set(key, row); else selected.delete(key); renderSelectedSummary(); }; });
    ...
    document.querySelector('#v37-nform').onsubmit = async function (event) {
      ...
      const form = new FormData(event.target), recipients = String(form.get('destinatario') || '').split(',').map(function (value) { return value.trim(); }).filter(Boolean), msg = document.querySelector('#v37-nmsg'), selectedRows = Array.from(selected.values());
      if (recipients.length < 1 || recipients.length > 4) { msg.textContent = 'Ingresa entre 1 y 4 destinatarios.'; return; }
      msg.textContent = 'Enviando...';
      const html = buildNotificationEmailHtml_(currentTypeLabel(), form.get('mensaje') || '', selectedRows);
      const first = selectedRows[0] || {};
      const result = await api('sendNotification', { notification: { tipo: currentTypeLabel(), asunto: form.get('asunto'), destinatario: recipients.join(','), zona: first.zona || '', rol: first.cargo || '', cc: form.get('cc'), mensaje: html, imeis: selectedRows.map(function (r) { return r.imei; }).filter(Boolean) }, userId: 'web-user' });
