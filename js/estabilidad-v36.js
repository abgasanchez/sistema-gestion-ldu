...
(function () {
  ...
  window.renderSettings = renderSettings = function () {
    ...
    document.querySelector('#v36-standardize').onclick = async function () { try { const dates = await api('formatDates', { userId: v36MaintUserId_ }); const money = await api('formatMoney', { userId: v36MaintUserId_ }); const upper = await api('uppercaseAll', { userId: v36MaintUserId_ }); alert((dates.message || 'Fechas normalizadas.') + ' ' + (money.message || 'Montos normalizados.') + ' ' + (upper.message || 'Mayúsculas aplicadas.')); } catch (error) { alert('No se pudo estandarizar: ' + error.message); } };
