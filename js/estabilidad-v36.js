...
(function () {
  ...
  window.renderSearch = renderSearch = async function () {
    ...
    try {
      ...
      const draw = function () {
        ...
        document.querySelector('#v36-cards').innerHTML = out.length ? '<div class="cards compact v36-card-grid">' + cardFields.map(function (field) { return '<div class="metric"><strong>' + cardValue(out[0], field[1]) + '</strong><span>' + field[0] + '</span></div>'; }).join('') + '</div>' : '';
      };
  ...
  if (typeof baseMapSection === 'function') window.mapSection = mapSection = function () {
    return baseMapSection().replace('<div id="v32-analysis-detail">', '<div class="analysis-actions"><button type="button" class="btn secondary" onclick="exportLduAnalysis()">📤 EXPORTAR</button></div><div id="v32-analysis-detail">');
  };
})();
